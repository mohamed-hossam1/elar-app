        ...
        initCart: async () => {
          ...
          try {
            if (user) {
              ...
              const guestId = getGuestId();
              if (guestId) {
                const cartResponse = await getCart(guestId);
        ...
        addToCart: async ({ variant, quantity = 1 }) => {
          ...
          try {
            if (user || !user) {
              ...
              let result;
              if (!currentItem) {
                result = await addToCart(variant.id, quantity, guestId);
        ...
        clearCart: async () => {
          ...
          try {
            if (user || !user) {
              ...
              const result = await clearCart(guestId);
              if (result && !result.success) {
                applyCart(originalCart);
                await get().initCart();
                return result;
