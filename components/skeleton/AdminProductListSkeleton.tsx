  ...
  return (
    <div className="animate-pulse">
      <div className="border border-black/10">
        <div className="grid grid-cols-[80px_1fr_120px_100px_100px] gap-4 p-4 border-b border-black/10 bg-black/5">
          {["ID", "Product", "Variants", "Stock", "Actions"].map((h, i) => (
            <span key={i} className="text-[11px] font-black uppercase tracking-wider text-black/40">{h}</span>
          ))}
        </div>
        {[...Array(8)].map((_, i) => (
          <div key={i} className="grid grid-cols-[80px_1fr_120px_100px_100px] gap-4 p-4 border-b border-black/10">
