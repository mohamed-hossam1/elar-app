        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-black/10">
            {products.map((product) => (
              <tr key={product.productId} className="hover:bg-black/[0.03] transition-colors group">
                <td className="text-left py-3 px-4">
                  <p className="font-bold truncate max-w-[200px]">{product.title}</p>
                  <p className="text-[10px] text-black/40 font-medium">ID: {product.productId.slice(0, 8)}</p>
                </td>
                <td className="text-right py-3 px-4 font-black">{product.unitsSold}</td>
                <td className="text-right py-3 px-4 font-bold">{formatCurrency(product.revenue)}</td>
          ...
          <tfoot className="bg-black/[0.02] border-t border-black/10">
            <tr className="font-black text-xs">
              <td className="py-3 px-4 uppercase tracking-tighter">Inventory Totals</td>
              <td className="text-right py-3 px-4">{totalUnits}</td>
              <td className="text-right py-3 px-4">{formatCurrency(totalRevenue)}</td>
