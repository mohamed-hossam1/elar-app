      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-2">
            <label className="text-[10px] uppercase font-black text-black/60 tracking-[0.2em] block">Full Name</label>
            <input
              className={`input-default ${errors.full_name ? 'border-red-500' : ''}`}
          ...
          <div className="space-y-2">
            <label className="text-[10px] uppercase font-black text-black/60 tracking-[0.2em] block">Phone Number</label>
            <input
              className={`input-default ${errors.phone ? 'border-red-500' : ''}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-2 relative">
            <label className="text-[10px] uppercase font-black text-black/60 tracking-[0.2em] block">City</label>
            <div className="relative">
              <select
                className={`input-default appearance-none bg-white transition-all ${errors.city ? 'border-red-500' : ''}`}
                value={formData.city}
          ...
          <div className="space-y-2">
            <label className="text-[10px] uppercase font-black text-black/60 tracking-[0.2em] block">Area / District</label>
            <input
              className={`input-default ${errors.area ? 'border-red-500' : ''}`}
        ...
        <div className="space-y-2">
          <label className="text-[10px] uppercase font-black text-black/60 tracking-[0.2em] block">Detailed Address</label>
          <input
            className={`input-default ${errors.address_line ? 'border-red-500' : ''}`}
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-black/5">
          <button
            type="submit"
            disabled={isPending}
            className="button-primary flex-1 gap-3 disabled:opacity-50"
          >
          ...
          <button
            type="button"
            onClick={onCancel}
            className="button-outline px-10 py-5"
          >
            Cancel
          </button>
        </div>
